...
public class ExcelUtil {
     ...
     * 初始化Excel表格
     *
     * @param filePath  存放excel文件的路径（/Export/demo.xls）
    ...
    public static void writeObjListToExcel(List<List<String>> objList, String fileName, Context c) {
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    List<String> list = (List<String>) objList.get(j);
                    for (int i = 0; i < list.size(); i++) {
                        sheet.addCell(new Label(j, i + 1, list.get(i), arial12format));
